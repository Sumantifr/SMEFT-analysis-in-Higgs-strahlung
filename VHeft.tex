...
% FIXME Instead of the following, make a nice list of UV models you actually constrain and list them in Section 5.
...
% FIXME -> I added the following information in the research hypothesis.
...
There is also the operator $H^\dagger H \bar{Q}\phi b$, which modifies the \PH--\Pb coupling, and thus changes $\PH \to \Pb \PAb$ decay rate. %FIXME What is its role? Why not in the table?
\begin{figure*}[hbtp]
\begin{center}
...
%FIXME One sentence about power counting and energy growth. Then come the angles below ...
...
% FIXME Here please describe concretely the angular amplitudes. Eq. 3.6 from Spannowsky, mention how these integrated to zero, and note the CP structure.
...
The operators ${\cal O}^{(1)}_{Hq}$, ${\cal O}_{Hu}$, ${\cal O}_{Hd}$, ${\cal O}_{HB}$, and ${\cal O}_{H\tilde{B}}$ affect only the $\PZ\PH$ production, because of the helicity structure involved in $\Pquark\APquark \to \PZ\text{,}\PW$ interaction. %FIXME Can this be made more concrete? What helicity structure is responsible? Also, a reference is needed. What is the purpose of this statement?

In the second objective, I use a the angular observables to further boost the sensitivity and tackle the CP-structure of the \PV--\PH interaction. As an example, I show on of the angular variables, $\phi$, in Fig.~\ref{fig:LHE_phi}. %FIXME <- Here a link to the sinusodial variations is needed from Sec. 3. It should be stated that sin/cos is CP odd/even. Let's add a sentence when the table is there.
...
The \PW process thus provides better statistical precision in the measurements. %FIXME Quote the x-sec numbers!
...
Amplitudes involving the longitudinal and transverse vector boson polarizations contribute differently to \VH production, in both the SM and in SM-EFT. The interference among helicity amplitudes, including CP effects, is preserved at the event level in subtle angular triple-correlations. %FIXME Specify when table is there.
...
The DeepAK8 tagger provides probability-normalized classifier values for the top quark, \PW boson, light quark or gluon hypothesis for each AK8 jet. We use this information later on. % FIXME Be specific
...
$<2$ additional AK4 jets & veto on extra b-tagged AK4 jets  \\%FIXME: Really "<" ?
...
the SM production, dominated by longitudinal polarization of \PW boson, exhibits differences in $\Theta$ for nonzero ${\cal O}_{HW}$  coefficients, because this operator  enhances the helicity amplitude of \PV\PH production with transverse polarizations.%FIXME Please check the correctness.
...
The CP-sensitivity in the $\phi$ observable is slightly affected by the ambiguities introduced by the unknown $z$ momentum of the neutrino entering the $\PW$ momentum reconstruction. %FIXME Suggest to motivate a selection MVA for the neutrino momentum as we do in tW reco ... adds an action item
...
As part of the analysis development, we will develop a multi-variate discriminator to select the correct solution based on the kinematic event properties. %FIXME Add a reference to a successful strategy.
...
%FIXME Add the following variables to the table. Add also a columns for ZH.
...
Normalizing the predicted yields to an integrated luminosity of 300\fbinv, we show the negative log-likelihood results in in Fig.~\ref{fig:NLL}. %FIXME Define q, is it profiled? 
\begin{figure*}[hbtp]
\begin{center}
...
%FIXME: We're not yet saying in the beginning that we solve energy growth with the boosted category!!
...
% Without the boosted category ... %FIXME <- Quantify the gain!
An improvement due to a binning in $\phi$ is obtained in the resolved category. 
The use of information about all three angles $\Theta$, $\theta$, $\phi$ is expected to result in a large increase in sensitivity. %FIXME Let us discuss this statement in person. Is phi weighted with sinxsin? 
