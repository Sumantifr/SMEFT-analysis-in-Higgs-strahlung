...
%For example, 
The proposer's group has recently developed  \textcolor{blue}{a new technique to design tree-based classifiers, which learn the likelihood ratio between SM-EFT and SM hypothesis exploiting the complete event information and optimally separate effects of SM-EFT operators from the SM background~\cite{Chatterjee:2021nms,Chatterjee:2022oco}. It has been already shown that this new technique provides a significant improvement in sensitvity to the SM-EFT operators under consideration with respect to traditional approaches of using a single kinematic distribution in regions optimzed for the SM signal~\cite{Chatterjee:2022oco}
}.
...
The SM-EFT effects in the \VH signal are incorporated with \texttt{MadWeight}~\cite{Artoisenet:2008zz} and using the \texttt{SMEFTsim}~model~\cite{Brivio:2017btx,Brivio:2020onw}. 
\textcolor{blue}{
We consider one opetator insertion at a time for the \VH production followed by the decay of \PH. 
In \texttt{SMEFTsim}~model, the correction to the total Higgs decay width is computed using separate $K$ factors for each decay channel following Ref.~\cite{Brivio:2019myy}.
}

\textcolor{blue}{
For the final measurement, both the SM \VH signal and the backgrounds will be generated at next-to-leading order in QCD using \texttt{POWHEG} or \texttt{Madgraph5\_aMC@NLO} and will be normalized using cross section calculated at the highest theoretical precision available. These event samples are produced centrally in the CMS Collaboration.
Electroweak corrections, important at high energy, will be applied to the SM \VH signal. 
The SM-EFT effects for quark initiated processes ($\PW\PH$ and $\PZ\PH$) will be calculated at LO using the \texttt{SMEFTsim}~model and at NLO with the \texttt{SMEFTatNLO}~model for the gluon-initiated process ($\PZ\PH$). 
The SM-EFT effects for the $\PZ\PH$ production with $\PZ \rightarrow  \Pl^{+}\Pl^{-}$ and $\PH \rightarrow \Pb\PAb$ decays are now known at the next-to-NLO (NNLO) in QCD for six operators~\cite{Haisch:2022nwz}. 
}
...
\end{center}
\caption{
Normalized distribution of polar angle $\Theta$ in $\PW\PH$ (top left) and $\PZ\PH$ (top right) signal samples for $c_{HW}=1$ and SM scenarios in the boosted category. 
Normalized distribution of azimuthal angle $\varphi$ in  $\PW\PH$ (bottom left) and $\PZ\PH$ (bottom right) productions for $c_{HW}=1$ and $c_{H\widetilde{W}}=1$, respectively; the SM component is subtracted from these distributions.
}
\label{fig:angles}
\end{figure*}
...
\begin{figure*}[t]
\begin{center}
\includegraphics[width=0.49\textwidth]{Figures/New/RECO/Limits/Full_NLL_WC_cpq3i_2019_opt1.png}
\includegraphics[width=0.49\textwidth]{Figures/New/RECO/Limits/Full_NLL_WC_cpW_2019_opt2.png}
\includegraphics[width=0.49\textwidth]{Figures/New/RECO/Limits/Full_NLL_WC_cpWtilde_2019_opt2.png}
\includegraphics[width=0.495\textwidth]{Figures/New/RECO/Selection_285.png}
\end{center}
\caption{
...
Also, simultaneous use of information about all three angles $\Theta$, $\theta$, $\varphi$ is expected to increase sensitivity significantly.
...
\small
  \begin{tabular}{l|m{0.9cm}|m{0.9cm}|m{0.9cm}|m{0.9cm}|m{0.9cm}|m{0.9cm}|m{0.9cm}|m{0.9cm}|m{0.9cm}|m{0.9cm}|m{0.9cm}|m{0.9cm}|m{0.9cm}|m{0.9cm}|m{0.9cm}|}
    \multirow{2}{*}{Task / Year} &
      \multicolumn{3}{c|}{2022 ($\int \mathcal{L}dt \sim 30$\fbinv)}  &
      \multicolumn{3}{c|}{2023 ($\int \mathcal{L}dt \sim 120$\fbinv)}  & 
      \multicolumn{3}{c|}{2024 ($\int \mathcal{L}dt \sim 160$\fbinv)} &
      \multicolumn{3}{c|}{2025} &
      \multicolumn{1}{c|}{2026} \\
      \hhline{~|-|-|-|-|-|-|-|-|-|-|-|-|-}
       & Apr-Jun & July-Sep & Oct-Dec & Jan-Apr & May-Aug & Sep-Dec & Jan-Apr & May-Aug & Sep-Dec & Jan-Apr & May-Aug & Sep-Dec & Jan-Apr \\
	\hhline{-|-|-|-|-|-|-|-|-|-|-|-|-|-}
    1. Signal simulation for Run~2 & \cellcolor{green} &   &  &  &  &  &  &  &  &  &  & &   \\
    \hline
    2. SM-EFT parameterization & \cellcolor{orange} &  &  &  &  &  &  &  &  &  &   &  &  \\
    \hline
    ...
    \hline
    8. Background estimation &  &  &  & \cellcolor{green} & & &  &  &  &  &   & &    \\
    \hline
    9. Multivariate analysis &  &  &  &  & \cellcolor{green}  & &  &  &  &  &  & &      \\
    \hline
    10. SM-EFT to SM separation &  &  & & & \cellcolor{orange} & & &  &  &  &    & &     \\
    \hline
    ...
    \hline
    13. SM-EFT effects in background &  & & & & & & \cellcolor{green} &  &  &  &   & &     \\
    \hline
    \hhline{~|~|~|~|~|~|~|~|~|~|~|~|~|~}
    ...
    \hline
    21. Combination of Run~2 and~3 results &  & & &  &  &  &  &  &  &   & & \cellcolor{green} &       \\
    \hline
    22. Publication of final results &  & & &  &  &  &  & &  &   &  & & \cellcolor{green}  \\
    \hline
  \end{tabular}
  \caption{
...
\item {\bf Signal simulation for the Run 2 analysis.} Simulate \VH event samples including the effects of SM-EFT operators both in production and decay using, e.g., \texttt{SMEFTsim}~\cite{Brivio:2020onw} at leading order or \texttt{SMEFT@NLO}~\cite{Degrande:2020evl} at next-to-leading order in perturbative QCD. 
...
\item {\bf SM-EFT parameterization.} Obtain polynomial parameterization of signal yields for arbitrary selections as a function of the Wilson coefficients. Validate the accuracy of the procedure with the dedicated simulation of event samples with non-zero Wilson coefficients.
\textcolor{blue}{
...
At NLO and NNLO, there are additional operators beyond the list presented in Table~\ref{Tab:Operators} affecting the VH production. Some of them, e.g., gluon--top and Higgs--top contact terms are expected to be probed with better sensitivity in other measurements (e.g., of \Pt\PAt, \Pt\PAt\PH productions) so we don't plan to consider those. 
Nevertheless, in the global combination, the results of this analysis might be useful to improve the sensitivity to those operators as well.
At NNLO, the Higgs--bottom quark chromomagnetic operator is expected to result in a significant change in the shape of important kinematic variables~\cite{Haisch:2022nwz} and we plan to include acceptance correction for that.
}
...
\item {\bf Neutrino momentum reconstruction using MVA.}
A finely tuned momentum reconstruction is required to resolve the ambiguities in neutrino momentum reconstruction. 
Using global event observables and kinematic properties of the reconstructed objects, we develop an MVA algorithm based on neural nets to select the correct solution. 
...
This will be a testbed of a method developed by the proposer's group using decision trees~\cite{Chatterjee:2021nms,Chatterjee:2022oco}. 
...
\textcolor{blue}{Systematic uncertainties on jet and lepton energy calibration affect MET determination, thus the reconstructed neutrino momentum. Those also affect the measurement of angular variabes. It is checked that those systematics don't reduce the power of the measurement significantly.}
...
\textcolor{blue}{
Gauge coupling operators ${\cal O}_{HW}$, ${\cal O}_{HWB}$, ${\cal O}_{HB}$ also affect the diboson production.
...
\textcolor{blue}
{
The mixing of SM-EFT operators through renormalization group equations can change the sensitivity results. 
The matrix of anomalous dimensions are theoretically known~\cite{Alonso:2013hga,Jenkins:2013wua,Jenkins:2013sda} and the effects are expected to be at the percent level.
These effects can be important if some poorly constrained operator leads to the running of strongly constrained one. 
However, the exisiting constrains on the SM-EFT operators considered in this proposal are generally weak, so these effects are expected to be small~\cite{Elias-Miro:2013eta}.
Nevertheless, there are on-going efforts in the theory community to check the RGE-effects on interpretation of LHC data in the context of global SM-EFT fits and we leave it to theorists.
}
...
\textcolor{blue}{Wolfgang Adam, a senior scientist at the HEPHY CMS group, will montior the progress of the project and provide necessary advice. He has a vast experience of reviewing CMS publications as the former Physics coordinator of the CMS Collaboration. His role will be purely advisory and he will not do any technical work.}
...
This project application requests funding for one PhD student for 48 months (EUR 39.780 per year in 2021). 
...
An extra $5\%$ of the total that amounts to EUR 180.990 is requested to cover other unexpected costs.  
Total requested funding sums up to EUR 190.039,50.  
...
\hline
General costs ($5\%$) & EUR & 9.049,50  \\
\hline
Total & EUR & 190.039,50
\end{tabular}
}
\end{center}
...
A search is being performed for additional Higgs bosons, predicted by minimal supersymmetric extensions of the standard model, in the $\tau\tau$ final state. I've measured the contribution of background processes where hadronic jets fake as $\tau$ leptons.~[ongoing]\\
...
\begin{itemize}
\item Eight parallel and three plenary talks at international conferences and workshops
...
\textbf{Andreas Gruber} (M.Sc. student at TU Wien, Thesis advisor: Dr. R. Sch{\"o}fbeck):\\
Designed and supervising his thesis project on \textit{lepton identification using a graph neural network}~(ongoing)\\
