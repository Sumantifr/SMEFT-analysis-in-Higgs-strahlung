...
the diagram with the modification with fermion--\PV coupling (Fig.~\ref{fig:Feynman_digarams}, middle) 
...
and that does not induce a cross section enhancement with energy. 
...
At high energy, since \PH has large \pt, its decay products are collimated and can be reconstructed using a single large-radius jet, thus providing a testbed for the application of the cutting-edge jet substructure techniques~\cite{Qu:2019gqs,Sirunyan:2020lcu}.
...
One point to note here is that the absence of knowledge about the direction of the incoming quark and anti-quark makes the terms in Eq.~\eqref{Eq:Amplitude} corresponding to $f^1_{TT}$, $f^1_{LT}$, and $\tilde{f}^1_{LT}$ vanish.
...
In this proposal, the sensitivity to the three most important operators at play is studied in detail. 
...
The experimental work to excavate two distinct features, change in cross section and CP nature of \PH--\PV coupling, will be tackled separately.
...
to boost the sensitivity of differential cross section measurements in the \VH production where \PV decays to leptons.
...
We target to use the combined data set collected by the CMS experiment during the LHC Run~2, corresponding to an integrated luminosity of 139\fbinv at $\sqrt{s}=13\TeV$, and the LHC Run~3, currently expected to be about $160\fbinv$.
...
We focus on the challenging $\PW\PH$ final states. The results are valid for the $\PZ\PH$ process, which serves as an independent channel with orthogonal SM-EFT sensitivity.
...
In summary, we consider the $\PW\PH$ and $\PZ\PH$ production processes with the $\PW\rightarrow\Pl\Pgn$, $\PZ\rightarrow{\Pl}^{+} {\Pl}^{-}$, and $\PH\rightarrow\Pb\PAb $ decay channels, where $\ell$ is either an electron or a muon.
%=(\Pe, \Pmu)$.
...
This allows extracting rich information about the event structure from a joint analysis of kinematic and angular variables.
%
...
The SM-EFT effects in background processes are ignored for this feasibility study 
since the operators considered here affect mainly the background from diboson production, which constitutes a very small background. This will be considered in the final measurement.
...
In Table~\ref{Tab:Regions}, the threshold values btag$^{\text{cut}}_{\text{max}}$ and btag$^{\text{cut}}_{\text{min}}$ are chosen to ensure a DeepJet \Pb tagging mistag rate of 1\% and 10\%  for light quark or gluon jets, respectively.
...
Here, the proposal is to probe SM-EFT operators in \VH production, where the gauge boson \PV decays to leptons. 
...
The work will be conducted within the CMS Collaboration. The reconstructed data and simulation, trigger information, object calibration factors provided centrally by the CMS Collaboration will be used. 
...
%Since the proposed work is on Higgs physics, which is one of the main centers of activities at LHC, the Ph.D. student will get high visibility within the CMS Collaboration.  
...
Information about SMEFT operators will be simulated for each event using a second-order polynomial in Wilson coefficients involved. 
%A number of weights will be stored for each event. 
...
The conditions used for lepton identification need to be optimized so that the variation of acceptance for different operators and the total systematic uncertainty are minimized while not compromising on the statistical power of data. 
...
The first one
%the working point-based method, 
requires using certain pre-defined centrally provided thresholds on \Pb tagging score of a jet to qualify as `\Pb-tagged', and the corresponding correction has small uncertainties. 
In the second approach, 
%the shape-based method, 
one is free to choose the threshold, optimal for the analysis, on \Pb tagging discriminator, and the corrections have large uncertainties. 
...
The threshold on the DNN-based \PH tagging condition needs to be optimized for signal-to-background separation. 
Correction factors need to be measured in appropriate control samples to match the tagging efficiency in data and simulation.
...
Corrections to match the trigger efficiency in simulation and data will be derived and applied to the simulation. 
%The same needs to be performed for the backup jet triggers. 

\item Neutrino reconstruction:

A careful analysis is required to assign the correct $\eta$ to the neutrino, which also benefits the CP-sensitivity of the \PH--\PW coupling measurements. 
...
%the backgrounds are primarily planned to be taken 
...
Different techniques for the background estimation, e.g., using the mass sidebands, will be checked.
...
In the feasibility study, the constraints on Wilson coefficients are derived just by using a few kinematic variables.
...
Systematic uncertainties related to different objects, e.g., leptons and jets, and corrections at event-level need to be applied. 
...
Systematic uncertainties in prediction from the choice of renormalization and factorization scales, parton distribution function, and the parton shower need to be derived. 
...
Improvements are expected in the Monte Carlo simulation of different processes in the next few years. Thus, the background estimation strategy to be developed for Run~2 analysis will need a validation for Run~3.
...
Results from the analyses based on Run~2 and Run~3 data samples will be combined. Final sensitivity on Wilson coefficients will be reported using the combined data set. 
...
The PI will spend $50\%$ of his time on this project. 
...
Tasks will be carried out in consultation with the leader of CMS data analysis group of the institute: Dr. Robert Sch{\"o}fbeck (RS), who is also performing SM-EFT measurements in top quark physics in association with a post-doctoral researcher Dr. Dennis Schwartz (DS). Interaction with RS and DS will be beneficial for this project. 
Particularly, SM-EFT parameterization (task 2) and SM-EFT to SM separation (task 10) will be performed together with RS and DS. 
